  ...
  const mockPrisma = {
    ...
    $transaction: (callback: (tx: any) => Promise<unknown> | unknown) => {
      const tx = {
        ...
        questionnaireResponse: {
          create: jest.fn().mockResolvedValue({ id: 'response-1', scoreTotal: 8, submittedAt: new Date().toISOString() })
        },
        surveyLink: {
...
describe('formsService.getFormByToken', () => {
  ...
  it('throws a 404 when the link does not exist', async () => {
    ...
    await expect(formsService.getFormByToken('missing')).rejects.toThrow(HttpError);
  });
  ...
  it('throws a 410 when the link is expired', async () => {
    mockSurveyLinkFindUnique.mockResolvedValue({
      ...
      maxResponses: null,
      usedCount: 0,
      version: { formJson: {}, template: { id: 'tpl', name: 'Demo', type: 'SURVEY' } },
      project: { id: 'project-1' }
    });

    await expect(formsService.getFormByToken('expired')).rejects.toThrow(HttpError);
  });
  ...
  it('returns minimal metadata when the link is valid', async () => {
    mockSurveyLinkFindUnique.mockResolvedValue({
      ...
      maxResponses: null,
      usedCount: 0,
      version: { formJson: { display: 'form' }, template: { id: 'tpl', name: 'Demo', type: 'SURVEY' } },
      project: { id: 'project-1' }
    });
