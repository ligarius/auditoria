...
import { computeReceptionTimes, ReceptionTimesInput } from './reception.metrics.js';
...
export const receptionService = {
  async list(projectId: string) {
    ...
    return receptions.map((r) => ({ ...r, metrics: computeReceptionTimes(r as ReceptionTimesInput) }));
  },

  async create(projectId: string, payload: any, userId: string) {
    const created = await prisma.reception.create({ data: { ...payload, projectId } });
    await auditService.record('Reception', created.id, 'CREATE', userId, projectId, null, created);
  ...
  async update(id: string, payload: any, userId: string) {
    ...
    const updated = await prisma.reception.update({ where: { id }, data: payload });
    await auditService.record('Reception', id, 'UPDATE', userId, before.projectId, before, updated);
  ...
  async remove(id: string, userId: string) {
    ...
    await auditService.record('Reception', id, 'DELETE', userId, before.projectId, before, null);
  },
  ...
  async metrics(projectId: string) {
    ...
    const totals = receptions.reduce<MetricsAccumulator>(
      ...
      { dwellSum: 0, dwellCount: 0, unloadSum: 0, unloadCount: 0, idleSum: 0, idleCount: 0 }
    );
    return {
