...
processRouter.post('/:projectId/:processId/file', requireProjectRole(['ConsultorLider', 'Auditor']), upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ message: 'Archivo requerido' });
  const saved = await fileService.save(req.params.projectId, req.file, req.user!.id);
  const processAsset = await processService.update(req.params.processId, { fileId: saved.id }, req.user!.id);
  res.json(processAsset);
});

processRouter.put('/:projectId/:processId', requireProjectRole(['ConsultorLider', 'Auditor']), async (req, res) => {
  const processAsset = await processService.update(req.params.processId, req.body, req.user!.id);
  res.json(processAsset);
});

processRouter.delete('/:projectId/:processId', requireProjectRole(['ConsultorLider']), async (req, res) => {
