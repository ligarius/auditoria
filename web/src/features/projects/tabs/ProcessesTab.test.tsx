...
describe('ProcessesTab', () => {
  ...
  it('renders the Recepción sub-tab when enabled', async () => {
    (api.get as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      data: { enabled: ['reception', 'picking'] },
    });
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project-a/procesos/reception']}>
        <Routes>
          <Route
            path="/projects/:id/procesos/:feature?"
            element={<ProcessesTab projectId="project-a" />}
          />
        </Routes>
      </MemoryRouter>
    );
    ...
    await waitFor(() =>
      expect(api.get).toHaveBeenCalledWith('/projects/project-a/features')
    );
  ...
  it('does not show Recepción when the feature is disabled', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project-b/procesos']}>
        <Routes>
          <Route
            path="/projects/:id/procesos/*"
            element={<ProcessesTab projectId="project-b" />}
          />
        </Routes>
      </MemoryRouter>
    );
    ...
    await waitFor(() =>
      expect(api.get).toHaveBeenCalledWith('/projects/project-b/features')
    );
  ...
  it('no longer fuerza la navegación a Procesos cuando se visita otro módulo', async () => {
    ...
    await waitFor(() =>
      expect(api.get).toHaveBeenCalledWith('/projects/project-c/features')
    );

    expect(screen.getByTestId('location')).toHaveTextContent(
      '/projects/project-c/kpis'
    );
  });
});
