...
export const computeReceptionTimes = (reception: ReceptionTimesInput) => {
  const dwell =
    reception.tArriveGate && reception.tExit
      ? dayjs(reception.tExit).diff(dayjs(reception.tArriveGate), 'minute')
      : null;
  const unload =
    reception.tUnloadStart && reception.tUnloadEnd
      ? dayjs(reception.tUnloadEnd).diff(
          dayjs(reception.tUnloadStart),
          'minute'
        )
      : null;
