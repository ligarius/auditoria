...
interface BuilderProps {
  ...
  onVersionCreated?: (version: { id: string; version: number; status: string }) => void;
}

type VersionState = { id: string; version: number; status: string } | null;

type TemplateState = { id: string; name: string; type: QuestionnaireType } | null;
    ...
    try {
      ...
      if (!currentTemplate) {
        const createdTemplate = await api
          ...
          .then((res) => res.data as { id: string; name: string; type: QuestionnaireType });
      ...
      const version = await api
        ...
        .then((res) => res.data as { id: string; version: number; status: string });

      setLastVersion(version);
      setStatusMessage(`Borrador #${version.version} guardado correctamente.`);
      onVersionCreated?.(version);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'No fue posible guardar el borrador.';
  ...
  const handlePublish = async () => {
    ...
    try {
      const version = await api
        ...
        .then((res) => res.data as { id: string; status: string; version: number });
      setLastVersion(version);
      setStatusMessage(`Versión ${version.version} publicada.`);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'No fue posible publicar la versión.';
      setErrorMessage(message);
    } finally {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Definición de la plantilla</h2>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <label className="flex flex-col text-sm font-medium text-slate-700">
            ...
            <select
              value={type}
              onChange={(event) => setType(event.target.value as QuestionnaireType)}
              className="mt-1 rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">Constructor de formulario</h2>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Reglas de scoring (JSON opcional)</h2>
        <p className="mt-1 text-sm text-slate-600">
          Puedes definir reglas declarativas para calcular el puntaje del formulario. El formato sigue la estructura
          <code className="mx-1 rounded bg-slate-100 px-1 py-0.5">{{"rules":[{{"id":"rule-1","questionId":"field1","operator":"eq","value":"Si","weight":10}}]}}</code>.
        </p>
        <textarea
          value={scoringDraft}
